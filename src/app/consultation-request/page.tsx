...
const FORM_TOAST_DURATION_MS = 2400;
...
function ConsultationArrivalToast({ visible }: { visible: boolean }) {
  return (
    <div
      role="status"
      aria-live="polite"
...
export default function ConsultationRequestPage() {
  ...
  const evaluateBarVisibility = useCallback(() => {
    if (typeof window === 'undefined') {
      return;
    }
    const matchesMobile = window.matchMedia(`(max-width: ${MOBILE_BAR_BREAKPOINT}px)`).matches;
    const threshold = window.innerHeight * MOBILE_SCROLL_THRESHOLD_MULTIPLIER;
    const shouldShow = matchesMobile && !formInViewRef.current && window.scrollY >= threshold;
    if (showBarRef.current !== shouldShow) {
      showBarRef.current = shouldShow;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        formInViewRef.current = isIntersecting;

        if (isIntersecting) {
          if (showBarRef.current) {
  ...
  useEffect(() => {
    const scrollToHashTarget = () => {
      ...
      const isFormSection = hash === '#form-section';
      pendingToastRef.current = isFormSection;
      setTimeout(() => {

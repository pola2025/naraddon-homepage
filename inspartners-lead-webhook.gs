...
// 통일된 설정값
const CONFIG = {
  SPREADSHEET_ID: '1L2NnwrNGULDt2M8upIH-VgxPUK5ei2gtVLjggfiDxDs',
  TELEGRAM: {
    BOT_TOKEN: '7947112373:AAGXL3AO9D8jkWnFkuUmU_VQbNpvOWHZREI',
    CHAT_ID: '-1002939393660'  // 인스파트너스 텔레그램 채팅방
  },
  EMAIL: {
    TO: 'yhk7205@naver.com',  // 인스파트너스 이메일
    BCC: 'mkt@polarad.co.kr',
    SUBJECT_PREFIX: '[인스파트너스] '
  },
  SENS: {
    ENABLED: false  // SMS 비활성화
  },
  WEBHOOK_SECRET: 'inspn2025secret',
  COMPANY_INFO: {
    NAME: '인스파트너스',
    CEO: '유희강',
    PHONE: '1666-6701',
    ADDRESS: '서울특별시 광진구 면목로 17, 302호',
    BIZ_NUMBER: '134-42-01132',
    HOMEPAGE: 'https://inspn.imweb.me/'
  }
};
...
function sendEmailNotification(data, rowNumber) {
  try {
    ...
    // HTML 이메일 본문 (인스파트너스 네이비 컬러 테마)
    const htmlBody = `
      <div style="font-family: 'Noto Sans KR', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #0B1E3A 0%, #173E73 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0;">
          <h2 style="margin: 0; font-size: 24px;">🏢 인스파트너스 신규 상담 신청</h2>
          <p style="margin: 10px 0 0 0; opacity: 0.95;">${platformEmoji} ${platform} 리드 광고 접수${data.adName ? ` - ${data.adName}` : ''}</p>
        </div>

        <div style="background: white; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
          <div style="background: rgba(11, 30, 58, 0.05); padding: 20px; border-radius: 8px; margin-bottom: 25px; border-left: 4px solid #C62828;">
            <h3 style="color: #0B1E3A; margin: 0 0 15px 0; font-size: 18px;">📋 접수 정보</h3>
            <table style="width: 100%;">
              <tr><td style="padding: 8px 0; color: #4C5260;">📅 접수일시</td><td>${safeString(data.timestamp)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">${platformEmoji} 플랫폼</td><td style="color: #173E73; font-weight: 600;">${platform}</td></tr>
              ${data.adName ? `<tr><td style="padding: 8px 0; color: #4C5260;">📢 광고명</td><td style="color: #6DA9E4; font-weight: 600;">${safeString(data.adName)}</td></tr>` : ''}
              <tr><td style="padding: 8px 0; color: #4C5260;">🏢 상호명</td><td style="font-weight: 600;">${safeString(data.company)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">👤 담당자</td><td>${safeString(data.name)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">📞 연락처</td><td style="color: #C62828; font-weight: 600;">${normalizePhoneNumber(data.phone)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">⏰ 상담희망시간</td><td style="color: #6DA9E4; font-weight: 600;">${safeString(data.consultTime) || '미선택'}</td></tr>
            </table>
          </div>

          <div style="background: #F5F7FA; padding: 20px; border-radius: 8px; margin-bottom: 25px; border-left: 4px solid #6DA9E4;">
            <h3 style="color: #0B1E3A; margin: 0 0 15px 0; font-size: 18px;">💼 사업 정보</h3>
            <table style="width: 100%;">
              <tr><td style="padding: 8px 0; color: #4C5260;">🏭 업종</td><td>${safeString(data.industry)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">📍 지역</td><td>${safeString(data.location)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">💼 사업자종류</td><td>${safeString(data.bizType)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">💵 직전년도매출</td><td>${safeString(data.revenue)}</td></tr>
              <tr><td style="padding: 8px 0; color: #4C5260;">💰 필요자금</td><td style="color: #C62828; font-weight: 700;">${safeString(data.amount)}</td></tr>
            </table>
          </div>

          <div style="text-align: center; margin-top: 30px;">
            <a href="https://docs.google.com/spreadsheets/d/${CONFIG.SPREADSHEET_ID}/edit#gid=0&range=${rowNumber}:${rowNumber}"
               style="display: inline-block; background: linear-gradient(135deg, #0B1E3A, #173E73);
        ...
        <div style="text-align: center; padding: 20px; background: #F5F7FA; border-radius: 0 0 10px 10px; color: #4C5260; font-size: 12px;">
          <p>인스파트너스 | ${CONFIG.COMPANY_INFO.PHONE}</p>
        </div>
      </div>
    `;
...
인스파트너스 | ${CONFIG.COMPANY_INFO.PHONE}
    `;
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .container {
          ...
          padding: 30px;
          border-radius: 10px;
          box-shadow: 0 2px 20px rgba(11, 30, 58, 0.15);
        }
        h1 {
          color: #0B1E3A;
          border-bottom: 3px solid #C62828;
        ...
        button {
          background: linear-gradient(135deg, #0B1E3A, #173E73);
          color: white;
          padding: 12px 30px;
          border: none;
          border-radius: 5px;
    ...
    <body>
      <div class="container">
        ...
        <div class="info-box" style="background: #F5F7FA; border-left-color: #6DA9E4;">
        ...
        <form id="testForm">
          ...
          <div class="form-group">
            ...
            <input type="text" name="webhook_secret" value="${CONFIG.WEBHOOK_SECRET}" readonly style="background: #F5F7FA;">
            <small style="color: #4C5260;">* Make 시나리오에서 이 값을 webhook_secret 필드로 전송해야 합니다</small>
          </div>

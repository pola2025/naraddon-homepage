...
export function middleware(request: NextRequest) {
  ...
  if (clientIp && BLACKLISTED_IPS.has(clientIp)) {
    console.log(`[Middleware] 블랙리스트 차단: ${clientIp} → ${pathname}`);
    return new NextResponse('Access Denied', { status: 403 });
  }

  // 1.5. IP 미식별 API 요청 차단 (프로덕션에서 IP 없으면 비정상)
  if (!clientIp && pathname.startsWith('/api/')) {
    console.log(`[Middleware] IP 미식별 API 차단: ${pathname}`);
    return new NextResponse('Access Denied', { status: 403 });
  }
  ...
  // 해외 IP 차단
  console.log(`[Middleware] 해외 IP 차단: ${clientIp} (${country}) → ${pathname}`);

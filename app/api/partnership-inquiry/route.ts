...
export async function POST(request: NextRequest) {
  try {
    const body: PartnershipInquiryRequest = await request.json();
    ...
    if (!botToken || !PARTNERSHIP_CHAT_ID) {
      console.error('❌ 텔레그램 환경변수 미설정:', { botToken: !!botToken, chatId: !!PARTNERSHIP_CHAT_ID });
      return NextResponse.json(
...
⏰ ${new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })}`;
    ...
    console.error('❌ 파트너십 제휴문의 전송 실패:', error);

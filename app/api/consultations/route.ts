...
// ================================================
// Cloudflare Worker를 통한 고객 이메일 발송
// Worker URL: naraddon-resend-mail.mkt9834.workers.dev
// ================================================
const RESEND_WORKER_URL = 'https://naraddon-resend-mail.mkt9834.workers.dev';

// Worker를 통해 고객 이메일 발송
async function sendCustomerEmailViaWorker(
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      console.error('[Worker] 이메일 발송 실패:', result.error);
      return { success: false, error: result.error };
    }

    console.log('[Worker] 고객 이메일 발송 성공:', customerEmail, result.id);
    return { success: true };
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ================================================
    // Cloudflare Worker를 통해 고객에게 확인 이메일 발송
    // ================================================
    let resendSuccess = false;
    if (consultation.userEmail) {
      const workerResult = await sendCustomerEmailViaWorker(
      ...
      resendSuccess = workerResult.success;
      if (!resendSuccess) {
        console.error('[Worker] 고객 이메일 발송 실패:', workerResult.error);
      }
    }

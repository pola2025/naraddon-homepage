...
const RESEND_FROM = process.env.RESEND_FROM || '나라똔 <noreply@mail.policy-fund.online>';
...
async function sendCustomerEmailViaResend(
  ...
  if (!RESEND_API_KEY) {
    console.warn('[Resend] API 키가 설정되지 않음');
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ================================================
    // Resend로 고객에게 확인 이메일 발송 (HTML 구조화된 이메일)
    // ================================================
    ...
    if (consultation.userEmail) {
      ...
      if (!resendSuccess) {
        console.error('[Resend] 고객 이메일 발송 실패:', resendResult.error);
      }
    }

  ...
  try {
    ...
    if (userId) {
      try {
        ...
        console.error('[policy-news][PUT] DB lookup failed:', error);
      }
    }

    // 권한 확인: 관리자(isAdmin 플래그 포함) 또는 기업심사관만 수정 가능
    const isAdminUser = checkIsAdmin(userForCheck);
    const isExaminer = userForCheck.role === 'examiner';

    if (!isAdminUser && !isExaminer) {
  ...
  try {
    ...
    if (userId) {
      try {
        ...
        console.error('[policy-news][DELETE] DB lookup failed:', error);
      }
    }

    // 권한 확인: 관리자(isAdmin 플래그 포함) 또는 기업심사관만 삭제 가능
    const isAdminUser = checkIsAdmin(userForCheck);
    const isExaminer = userForCheck.role === 'examiner';

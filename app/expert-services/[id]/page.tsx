  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 429 || data.code === 'RATE_LIMITED') {
        setSubmitMessage({
          type: 'error',
          text: '⚠️ 짧은 시간 내 너무 많은 접수가 감지되었습니다. 잠시 후 다시 시도해주세요.',
        });
        return;
      }
      ...
      if (data.success) {
        setSubmitMessage({
          type: 'success',
          text: '상담 신청이 완료되었습니다. 빠른 시일 내에 연락드리겠습니다.',
        });
        // 폼 초기화 (전문가 이름은 유지)
        setFormData({
          companyName: '',
        ...
        setSubmitMessage({
          type: 'error',
          text: data.error || '상담 신청에 실패했습니다. 다시 시도해주세요.',
        });
      }
    } catch (error) {
    ...
    return (
      <div className="expert-detail-page">
        <div className="error-container">
          ...
          <Link href="/expert-services" className="btn-back">
            목록으로 돌아가기
          </Link>
        </div>
      </div>
              ...
              <div className="profile-header">
                ...
                <div className="profile-info">
                  <div className="profile-info-top">
                    <span className="profile-badge">{expert.position || '나라똔 인증 전문가'}</span>
            ...
            <div className={`tab-panel ${activeTab === 'about' ? 'active' : ''}`}>
              <div className="about-section">
                ...
                <div className="about-content">
                  ...
                  {expert.detailedIntro && (
                    <p style={{ marginTop: '16px' }}>{expert.detailedIntro}</p>
                  )}
                  ...
                  <div className="naraddon-logo-container">
                    <img src="/images/ttontok-logo.png" alt="나라똔" className="naraddon-logo" />
                  </div>
                </div>
              </div>
            </div>
                  ...
                  {expert.galleryImages && expert.galleryImages.length > 0 ? (
                    expert.galleryImages.map((imageUrl, idx) => (
                      <div key={idx} className="gallery-image-item">
                        <img src={imageUrl} alt={`${expert.name} 자료 ${idx + 1}`} loading="lazy" />
                      </div>
                    ))
                  ) : (
              ...
              <div className="case-grid">
                {expert.successCases && expert.successCases.length > 0 ? (
                  ...
                  <div className="case-card">
                    ...
                    <p className="case-description">전문가의 성공 케이스는 현재 준비 중입니다.</p>
                  </div>
                )}
              </div>
            </div>
